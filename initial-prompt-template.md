...
- This repo was created from the `project-scaffold` template. **Milestone 0
  = run the `/milestone-0` skill** — it resolves every `TODO(milestone-0)`
  marker (CI test command, Dockerfile, compose file, .env.example, doc
  skeletons copied into place from `templates/`) and then deletes itself
  and `templates/`. The release workflow refuses to run while any marker
...
- Releases are driven by the `/release` skill, which holds the canonical
  release checklist; it ends by pasting that checklist into the
  conversation and ticking every item (compose pin bump included).
  Checklists survive context loss; memory doesn't.
...
- Never commit secrets, tokens, runtime logs, or `data/`. `CLAUDE.md` and
  `DECISIONS.md` are **committed** (so clones, worktrees, and CI/cloud
  agent sessions have the project rules) — keep them public-safe.
  `source-files/` and `HANDOVER.md` are gitignored and live only in this
  OneDrive-synced folder; anything sensitive (real IPs, environment
  details, credentials) goes there or in `.env`, never in committed files.
...
- Worktrees: for risky refactors or parallel sub-agent build work, use a
  git worktree at `C:\worktrees\<repo-name>\<branch>` — always **outside
  OneDrive**. Policy details live in `CLAUDE.md`. Nothing releases from a
  worktree; merge back to `main` and release from there.
- Assume the session can die at any moment (5-hour usage limit, crash,
  machine switch). Follow the interruption-resilience rules in `CLAUDE.md`:
  push every green step, `/checkpoint` after each unit of work and before
  large fan-outs, announce big fan-outs so I can defer them (I can see the
  usage meter; you can't), and keep HANDOVER's `Next action:` line current
  so any restart resumes in minutes.

## 9. Rules — documentation and handover

- Documentation is a write/audit pair of sub-agents, defined in
  `.claude/agents/` (committed): `docs-writer` (fast model) drafts the
  README, CHANGELOG, and wiki; `docs-auditor` (stronger model) audits for
  leakage, accuracy against the code, and version alignment. Run the pair
  before every release (the `/release` skill does this). Model choices live
  in those agent files only — update them there as models change.
- Use the `scout` sub-agent to digest large inputs (`source-files/`
  exports, third-party API docs) into briefs instead of reading them into
  the main session's context.
- Run `code-auditor` on every sub-agent build branch before merging it to
  `main`, and before any release containing code changes. It checks this
  project's contract (tier triggers, test fidelity, leakage, version
  discipline); generic bug-hunting stays with the built-in `/code-review`.
- Spin off as many sub-agents (and worktree-isolated agents) as the task
  warrants — parallel work, independent review, context protection — at
  your own judgment, picking models by task weight (cheap/fast for
  mechanical work, strongest for review). Only the docs write/audit pair
  has pinned models. Delegation never crosses an unapproved milestone
  boundary.
- The wiki (exhaustive, easy to use) is also written by `docs-writer` and
  audited by `docs-auditor`. Where it lives depends on repo visibility:
...
- Copy `templates/HANDOVER.md` to the repo root during milestone 0 and keep
  ...
  top. When I say I'm wrapping up, run the `/wrap-up` skill, which drives
  that checklist explicitly and shows me the ticks. Update HANDOVER.md
  before ending ANY session that changed anything.
- Copy `templates/DECISIONS.md` to the repo root during milestone 0 (it is
  committed — keep entries public-safe). Decisions, chosen trade-offs, and
  hard-won lessons are **append-only** entries there — never rewritten,
  never trimmed. HANDOVER.md churns every session; DECISIONS.md only
  grows. Cross-reference instead of duplicating.
- Copy `templates/CLAUDE.md` to the repo root during milestone 0 (also
  committed) and grow it with the project's binding engineering rules as
  they emerge. It holds the testing-tier table, the approved milestone
  plan, the worktree policy, and the delegation rules (the release
  checklist itself lives in the `/release` skill).
