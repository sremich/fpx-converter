...
> context for AI agents working on this project from any of Stevie's
> machines. It is deliberately **gitignored** and lives only in the
> OneDrive-synced project directory. Local agent memory
> (`~/.claude/projects/...`) does not follow Stevie between machines; this
...
Run this explicitly (and show the ticks) whenever Stevie says they're
...
- [ ] If a release happened: compose pin bumped, `/release` skill's checklist completed
...
- [Anything in flight or half-done, and dead ends already ruled out.]
- Next action: [the exact next command or edit — the resume point if this
  session dies at the usage limit]
...
5. [Where credentials come from (`.env` — ask Stevie; never in chat/repo).]
